# Aether-Eye: An Integrated Satellite Intelligence Platform for Automated Critical Infrastructure Monitoring with Multi-Modal OSINT Correlation
...
1. An **end-to-end automated pipeline** from STAC discovery through change detection, event generation, temporal baseline anomaly scoring, and multi-channel alerting.
2. A **SiameseUNet change detection model** trained on 1,134 paired satellite tiles achieving a test IoU of 0.8243, competitive with published methods on comparable datasets.
3. A **ConvNeXt Small military aircraft classifier** covering 98 distinct airframe variants across 34,718 training chips, achieving 68.57% top-1 accuracy via a novel two-phase fine-tuning methodology.
4. A **three-pass scored OSINT geo-tagging algorithm** correlating 21 curated RSS sources to 18 globally monitored sites using tag matching, alias resolution, and country-military context boosting.
5. **INT8 dynamic quantization** achieving 3.7–3.96× model compression, reducing the total inference footprint to 81 MB for free-tier cloud deployment within 512 MB RAM constraints.
6. A **complete open-source system** deployable via a single `docker compose` command at zero recurring cost.

---

## 2. Related Work

### 2.1 Satellite Change Detection
...
### 2.2 Fine-Grained Aircraft Classification
...
### 2.3 OSINT and Geospatial Intelligence Systems
...
### 2.4 Edge and Resource-Constrained Deployment
...
Layer 1 — Data Ingestion:
  Sentinel-2 STAC → Scene Processor → GeoTIFF Tiler (512×512 tiles)
  OpenSky Network → ADS-B flight states
  RSS feeds (21 sources) → OSINT articles

Layer 2 — Model Inference (ONNX Runtime, no PyTorch in production):
  SiameseUNet → change mask per tile pair
  ConvNeXt Small → aircraft class per crop
  YOLOv8n → aircraft bounding boxes (beta)

Layer 3 — Intelligence Synthesis:
  Event Engine → temporal baseline comparison → tiered alerts
  OSINT geo-tagger → site association scoring
  Site Aggregator → per-site daily event counts

Layer 4 — Visualization:
  Next.js 15 dashboard, MapLibre GL, 18-site global map
```

This layered design ensures that each component can be independently tested, updated, and scaled. The inference layer operates without PyTorch dependencies in production—all models are exported to ONNX format and executed via ONNX Runtime, eliminating the ~2 GB PyTorch installation footprint.

### 3.2 Satellite Scene Pipeline
...
The production variant uses an ImageNet-pretrained ResNet-34 backbone (`SiameseUNetResNet34`) for the shared encoder, providing robust low-level feature extraction that transfers effectively to satellite imagery domains.
...
| Average change pixel ratio | ~9.4% |
| Unchanged pixel ratio | ~90.6% |
...
### 4.3 Loss Function — Hybrid Tversky
...
| Config | Loss Function | Augmentation | Val IoU (5 epochs) | Observation |
|--------|--------------|--------------|---------------------|-------------|
...
## 5. Aircraft Classification Module

### 5.1 Architecture — ConvNeXt Small
...
where z_k is the k-th logit output.
...
- **5th-generation fighters:** F-22 Raptor, F-35, Su-57, J-20, J-35, KAAN, KF-21 Boramae
- **4th-generation fighters:** F-16A/B, F/A-18, MiG-29, Su-34, Rafale, Eurofighter Typhoon, JAS-39 Gripen
- **Strategic bombers:** B-2 Spirit, B-1 Lancer, B-52, Tu-160, Tu-95, H-6
- **Transport/tanker:** C-17 Globemaster, C-130, C-5 Galaxy, KC-135, An-124, Y-20
- **Rotary-wing:** AH-64 Apache, Ka-52 Alligator, Mi-28, UH-60 Black Hawk, CH-47 Chinook
- **UAVs:** MQ-9 Reaper, RQ-4 Global Hawk, Bayraktar TB2, Bayraktar Akinci, KIZILELMA
...
- Train only the classification head (`model.head`, ~76K parameters)
...
**Phase 2 — Full Fine-Tuning (50 epochs):**
...
Empirical Grad-CAM analysis confirms that the model attends to aircraft-specific geometric features (wing sweep angle, fuselage cross-section, tail configuration, engine placement) rather than background artifacts such as runway markings or taxiway patterns. This capability is exposed via the `/v1/aircraft-gradcam` REST endpoint, returning a base64-encoded heatmap overlay for any classified image.

### 5.5 Results

| Metric | Value |
|--------|-------|
| Val Top-1 Accuracy | **68.57%** |
...
### 5.6 INT8 Quantization for Deployment
...
|-------|-----------|-----------|-------------|-------------|
| SiameseUNet (change) | 116 MB | 31 MB | 3.74× | <0.5% IoU |
| ConvNeXt Small (aircraft) | 198 MB | 50 MB | 3.96× | <1% accuracy |
| **Combined** | **314 MB** | **81 MB** | **3.88×** | **Negligible** |
...
## 6. OSINT Intelligence Correlation

### 6.1 Ingestion Pipeline

Aether-Eye ingests articles from 21 curated RSS sources organized into three reliability tiers:

- **Tier 1 (wire services):** BBC News, Sky News, Reuters World
- **Tier 2 (specialized/regional):** Breaking Defense, Al Jazeera, Arab News, The National UAE, Middle East Eye, Defense News, Janes Defense, Al Arabiya English, Times of Israel, Haaretz, Gulf News, Japan News, Stars and Stripes
- **Tier 3 (niche/enthusiast):** Task and Purpose, The Aviationist, Naval News, Milavia, Scramble Magazine

The ingestion service polls all sources every 30 minutes via APScheduler. Each source is wrapped in an independent try/except block ensuring that one failed source does not block the remaining feeds. Articles are deduplicated via a UNIQUE constraint on the URL column using PostgreSQL's `ON CONFLICT DO NOTHING` upsert semantics. Published timestamps are parsed from RSS `published`, `updated`, or `created` fields with timezone normalization to UTC.

### 6.2 Three-Pass Geo-Tagging Algorithm

Each ingested article is scored against all 18 monitored sites through a three-pass algorithm:

**Pass 1 — Exact tag match (+2 per match):**
Each site maintains a curated list of identifying tags. For example, Al Udeid Air Base tags include `["Al Udeid", "CENTCOM", "Qatar air base", "55th Wing", "379th", "Air Operations Center"]`. When any tag appears in the article's title + summary text, the associated site receives +2 points.

**Pass 2 — Alias map (+1 per match):**
A broader alias dictionary maps colloquial or regional terms to site identifiers. For example, `"bandar abbas"`, `"hormuz"`, or `"persian gulf blockade"` all map to `strait_hormuz_north` (Bandar Abbas Port). Each matched alias contributes +1 point.

**Pass 3 — Country + military context (+1):**
If the article contains both a country name and any military term from a predefined vocabulary (`[military, air force, navy, troops, strike, missile, drone, exercise, deployment, base]`), all sites in that country receive +1 point.
...
**Limitation:** The 75% untagged rate reflects a fundamental challenge—major wire services cover geopolitics broadly without naming specific installations. An article discussing "US military operations in the Middle East" contains insufficient specificity for site-level attribution.

**Improvement path:** Named Entity Recognition (NER) using spaCy or similar NLP models could extract location mentions beyond keyword matching, combined with fuzzy matching and geocoding to associate extracted place names with monitored site coordinates.
...
| Alert Tier | Condition | Interpretation |
|------------|-----------|----------------|
| NEW_OBJECT | baseline = 0 and detections > 0 | First-ever detection at this site |
| ELEVATED_ACTIVITY | current ≥ 1.5× baseline | Moderate deviation from norm |
| ACTIVITY_SURGE | current ≥ 3.0× baseline | Significant anomaly requiring attention |
...
|-----------|---------------|-----|-------|
| Change detection (per scene) | ~3 s | ~11.6 s | 484 tiles at 512×512 |
...
| Git LFS size | ~82 MB (INT8 models only) |
| Docker image size | ~1.2 GB (including ONNX Runtime) |
...
The military aircraft classification capability raises dual-use considerations. While the classifier can identify aircraft types from imagery, it operates on publicly available data at 10 m resolution—a spatial resolution at which individual aircraft are typically not resolvable in Sentinel-2 imagery. The classifier is primarily intended for use with higher-resolution imagery obtained from public sources such as Google Earth or openly shared satellite photographs. Responsible disclosure of the model's capabilities and limitations is addressed through this publication.
...
[1] R. C. Daudt, B. Le Saux, and A. Boulch, "Fully Convolutional Siamese Networks for Change Detection," in *Proc. IEEE Int. Conf. Image Processing (ICIP)*, Athens, Greece, 2018, pp. 4063–4067.
...
[3] W. G. C. Bandara and V. M. Patel, "A Transformer-Based Siamese Network for Change Detection," in *Proc. IEEE Int. Geoscience and Remote Sensing Symposium (IGARSS)*, Kuala Lumpur, Malaysia, 2022, pp. 207–210.

[4] Z. Liu, H. Mao, C.-Y. Wu, C. Feichtenhofer, T. Darrell, and S. Xie, "A ConvNet for the 2020s," in *Proc. IEEE/CVF Conf. Computer Vision and Pattern Recognition (CVPR)*, New Orleans, LA, USA, 2022, pp. 11976–11986.
...
[6] R. R. Selvaraju, M. Cogswell, A. Das, R. Vedantam, D. Parikh, and D. Batra, "Grad-CAM: Visual Explanations from Deep Networks via Gradient-Based Localization," in *Proc. IEEE Int. Conf. Computer Vision (ICCV)*, Venice, Italy, 2017, pp. 618–626.

[7] S. S. M. Salehi, D. Erdogmus, and A. Gholipour, "Tversky Loss Function for Image Segmentation Using 3D Fully Convolutional Deep Networks," in *Machine Learning in Medical Imaging (MLMI)*, Springer LNCS, vol. 10541, 2017, pp. 379–387.

[8] K. Leetaru and P. A. Schrodt, "GDELT: Global Data on Events, Location and Tone, 1979–2012," in *ISA Annual Convention*, San Francisco, CA, USA, 2013.

[9] D. Lam, R. Kuzma, K. McGee, S. Doober, M. Laielli, M. Klaric, Y. Buber, and B. McCord, "xView: Objects in Context in Overhead Imagery," *arXiv preprint arXiv:1802.07856*, 2018.
