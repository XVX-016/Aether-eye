...
\hypertarget{comparative-baseline-analysis}{%
\section{3. Comparative Baseline Analysis}\label{comparative-baseline-analysis}}

\hypertarget{change-detection-baselines}{%
\subsection{3.1 Change Detection Baselines}\label{change-detection-baselines}}

Direct, quantitative comparison against published remote sensing change detection models is complicated by dataset differences: Aether-Eye is evaluated on the Building-change dataset, whereas the dominant literature baseline is LEVIR-CD [2]. Nevertheless, to contextualize architectural performance, Table~I compares our SiameseUNet against leading models.
...
ChangeFormer [3] & IGARSS 2022 & LEVIR-CD & MiT-B0 & 0.823 \\
\textbf{Aether-Eye SiameseUNet} & \textbf{Our Work} & \textbf{Building-change} & \textbf{ResNet-34} & \textbf{0.8243} \\
\end{longtable}

Our model achieves an IoU of 0.8243 on the Building-change dataset. Although LEVIR-CD and Building-change are distinct, their structural scale, spatial resolution (0.5--1.0 m/pixel), and density of building targets are highly comparable. Our convolutional design yields competitive accuracy while requiring substantially lower computational complexity than ViT-based architectures like BIT or ChangeFormer.

\hypertarget{aircraft-classification-baselines}{%
\subsection{3.2 Aircraft Classification Baselines}\label{aircraft-classification-baselines}}

Table~II compares our classification performance against standard fine-grained visual classification (FGVC) baselines.

\begin{longtable}[]{@{}lllll@{}}
\toprule\noalign{}
Model & Pretraining & Target Dataset & Classes & Top-1 \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
ResNet-50 [5] & ImageNet & FGVC-Aircraft & 100 & 88.0\% \\
ViT-B/16 & ImageNet & FGVC-Aircraft & 100 & 91.3\% \\
ConvNeXt-B & ImageNet & FGVC-Aircraft & 100 & 90.1\% \\
Aether-Eye v1 (FGVC) & ImageNet & FGVC-Aircraft & 100 & 72.5\% \\
\textbf{Aether-Eye v2 (Military)} & \textbf{v1 FGVC} & \textbf{Military Dataset} & \textbf{98} & \textbf{68.57\%} \\
\end{longtable}

The Aether-Eye v2 classifier trades a small margin of absolute accuracy for domain relevance. Standard FGVC-Aircraft models are trained exclusively on high-resolution civilian aircraft oblique photography. Our v2 model successfully resolves 98 military-specific variants from mixed oblique and nadir viewing angles, providing the specific class resolution required for security and defense monitoring applications.
...
\begin{figure}[t]
\centering
\resizebox{\columnwidth}{!}{\input{docs/figures/fig1_architecture}}
...
\subsection{4.2 Satellite Scene Pipeline}\label{satellite-scene-pipeline}}
...
\section{5. Change Detection Module}\label{change-detection-module}}

\hypertarget{architecture-siameseunet}{%
\subsection{5.1 Architecture --- SiameseUNet}\label{architecture-siameseunet}}

The change detection subsystem employs a SiameseUNet architecture (see Fig.~\ref{fig:siameseunet_arch}) consisting of a dual-branch shared-weight encoder and a U-Net decoder. Given a temporal image pair (I₁, I₂) representing ``before'' and ``after'' states of the same geographic region, the shared encoder extracts hierarchical feature maps F₁ and F₂ at multiple spatial scales (H, H/2, H/4, H/8).
...
\begin{figure}[htbp]
\centering
\resizebox{0.9\columnwidth}{!}{\input{docs/figures/fig2_siameseunet}}
\caption{Detailed architecture of the SiameseUNet model.}
\label{fig:siameseunet_arch}
\end{figure}

\hypertarget{training-dataset}{%
\subsection{5.2 Training Dataset}\label{training-dataset}}
...
\subsection{5.3 Loss Function --- Hybrid Tversky}\label{loss-function-hybrid-tversky}}
...
\subsection{5.4 Ablation Study}\label{ablation-study}}
...
\hypertarget{results}{%
\subsection{5.5 Results}\label{results}}
...
The test IoU (0.8243) exceeding the validation IoU (0.7936) is attributed to distributional similarity between the training and test splits of the Building-change dataset (see Fig.~\ref{fig:cd-iou-curve}). Data leakage was verified absent through fixed-seed splitting and geographic non-overlap checks.
...
\section{6. Aircraft Classification Module}\label{aircraft-classification-module}}

\hypertarget{architecture-convnext-small}{%
\subsection{6.1 Architecture --- ConvNeXt Small}\label{architecture-convnext-small}}
...
\subsection{6.2 Dataset --- Military Aircraft Dataset}\label{dataset-military-aircraft-dataset}}
...
\subsection{6.3 Two-Phase Fine-Tuning Methodology}\label{two-phase-fine-tuning-methodology}}

Transfer learning from the v1 FGVC-Aircraft checkpoint (100 civilian/mixed classes, 72.5\% top-1) to the v2 Military Aircraft Dataset (98 military classes) follows a two-phase protocol (see Fig.~\ref{fig:finetune}):
...
Figure~\ref{fig:aircraft-acc-curve} shows the Phase~2 validation accuracy trajectory. The model crosses 50\% accuracy by epoch~12, 60\% by epoch~20, and reaches its peak of 68.57\% at epoch~48. The smooth convergence curve---achieved only with layer-wise LR scheduling---contrasts with the immediate accuracy collapse observed under uniform LR (see Fig.~\ref{fig:finetune}, center). The model's 71.99\% macro F1 score confirms consistent performance across all 98 classes, not just the numerically dominant fighter category.
...
\begin{figure*}[t]
\centering
\resizebox{\textwidth}{!}{\input{docs/figures/fig3_finetune}}
...
\subsection{6.4 Grad-CAM Explainability}\label{grad-cam-explainability}}
...
\subsection{6.5 Results}\label{results-1}}
...
\subsection{6.6 INT8 Quantization for Deployment}\label{int8-quantization-for-deployment}}
...
\section{7. OSINT Intelligence Correlation}\label{osint-intelligence-correlation}}

\hypertarget{ingestion-pipeline}{%
\subsection{7.1 Ingestion Pipeline}\label{ingestion-pipeline}}
...
\subsection{7.2 Three-Pass Geo-Tagging Algorithm}\label{three-pass-geo-tagging-algorithm}}
...
\subsection{7.3 Coverage Analysis}\label{coverage-analysis}}
...
\section{8. Global Monitoring System}\label{global-monitoring-system}}

\hypertarget{site-registry}{%
\subsection{8.1 Site Registry}\label{site-registry}}
...
\subsection{9.2 Temporal Baseline Anomaly Detection}\label{temporal-baseline-anomaly-detection}}
...
\subsection{8.3 ADS-B Flight Activity}\label{ads-b-flight-activity}}
...
\section{9. System Evaluation}\label{system-evaluation}}

\hypertarget{change-detection-performance}{%
\subsection{9.1 Change Detection Performance}\label{change-detection-performance}}
...
\subsection{9.2 Aircraft Classification Performance}\label{aircraft-classification-performance}}
...
\subsection{9.5 Deployment Footprint}\label{deployment-footprint}}
...
\subsection{10.1 Domain Shift in Aircraft Classification}\label{domain-shift-in-aircraft-classification}}
...
\subsection{10.2 OSINT Geo-Tagging Coverage}\label{osint-geo-tagging-coverage}}
...
\subsection{10.3 Sentinel-2 Latency}\label{sentinel-2-latency}}
...
\subsection{10.4 Scalability}\label{scalability}}
...
\subsection{10.5 Ethical Considerations}\label{ethical-considerations}}
...
Complete implementation, trained model artifacts (INT8 ONNX), dataset loaders, training scripts, and API documentation are openly available at \url{https://github.com/XVX-016/Aether-eye} under a permissive research license.
